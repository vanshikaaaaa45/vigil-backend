...
exports.list = async (req, res) => {
  try {
    const { rows } = await query(
      ...
      [req.dataUserId || req.user.id]   // ← use team owner's keys
    );
    res.json({ keys: rows });
...
exports.create = async (req, res) => {
  try {
    ...
    const { rows } = await query(
      ...
      [req.user.id, name, hash, prefix, permissions]  // always create under own account
    );
    res.status(201).json({
      key:    rows[0],
      rawKey: raw,
...
exports.remove = async (req, res) => {
  try {
    const { rows } = await query(
      ...
      [req.params.id, req.user.id]  // can only delete own keys
    );
